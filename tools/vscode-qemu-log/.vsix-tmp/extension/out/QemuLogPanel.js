...
class QemuLogPanel {
    ...
    // -------------------------------------------------------------------------
    stop()    { this._stopQemu(); }
    restart() { this._stopQemu(); this._startQemu(); }
    ...
    _startQemu() {
        ...
        const bin    = cfg.get("qemuBinary", "/home/a/qemu/build/qemu-system-aarch64");
        const kernel = this._resolveVars(
            cfg.get("kernelImage", "${workspaceFolder}/build/kernel8.img"),
        );
        const extraArgs = cfg.get("qemuArgs", [
        ...
        try {
            ...
            // Line-buffer stdout — chunks may not align on newlines
            this._qemu.stdout?.on("data", (chunk) => {
                this._lineBuffer += chunk.toString("utf8");
                const lines = this._lineBuffer.split("\n");
                this._lineBuffer = lines.pop() ?? "";
                for (const line of lines) { this._postLog(line); }
            });

            this._qemu.stderr?.on("data", (chunk) => {
                for (const line of chunk.toString("utf8").split("\n")) {
                    if (line.trim()) { this._postLog("[qemu-stderr] " + line); }
                }
            });

            this._qemu.on("close", (code) => {
            ...
            this._postStatus(`Failed to start QEMU: ${err.message}`);
        }
    }
    ...
    _sendConfig() {
        ...
        this._panel.webview.postMessage({
            type:       "config",
            colorRules: cfg.get("colorRules", []),
            maxLines:   cfg.get("maxLines", 20000),
        });
    }

    _postLog(line)   { this._panel.webview.postMessage({ type: "log",    line }); }
    _postStatus(text){ this._panel.webview.postMessage({ type: "status", text }); }
    ...
    // -------------------------------------------------------------------------
    // Webview HTML (all inline — no external media files needed)
    // -------------------------------------------------------------------------
    _buildHtml() {
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .stop-btn:hover { background: #6e1414; border-color: #f44747; }
    ...
    #empty-msg { color: #555; padding: 14px; text-align: center; display: none; }

    /* ── Legend ──────────────────────────────────────────────────────────── */
    #legend-bar {
      display: flex; align-items: center;
      gap: 4px 12px; padding: 3px 8px;
      background: #252526; border-top: 1px solid #3c3c3c;
      flex-shrink: 0; flex-wrap: wrap;
    }
    #legend-bar.collapsed { display: none; }
    .leg-item { display: flex; align-items: center; gap: 4px; font-size: 10px; color: #858585; }
...
<body>
  <div id="toolbar">
    ...
    <button class="btn" id="btn-wrap"    title="Toggle line wrap">Wrap</button>
  ...
  <script>
    ...
    // Strip ANSI CSI escape sequences produced by some QEMU output
    ...
    function matchRule(line) {
      for (const r of colorRules) {
        ...
        } catch (_) { /* skip invalid regex in rule */ }
      }
      return null;
    }
    ...
    function applyVisibility(el, text) {
    ...
    function flushPending() {
      ...
      // Drop oldest lines when over the configured maximum
    ...
    function syncEmptyMsg() {
      if (!logOutput.childElementCount) { emptyMsg.style.display = 'none'; return; }
    ...
    function buildLegend() {
      ...
      for (const r of colorRules) {
        ...
        // Make the stored regex pattern readable in the legend
    ...
    document.getElementById('filter-clear').addEventListener('click', () => {
      filterEl.value = ''; rebuildFilter(); filterEl.focus();
    });
    ...
    window.addEventListener('message', ({ data }) => {
      switch (data.type) {
        case 'log':
          if (!paused) { pending.push(data.line); scheduleFlush(); }
          break;
        case 'status':
        ...
        case 'config':
          ...
          buildLegend();
          // Re-apply colors to lines already in the DOM
          for (const el of logOutput.children) {
    ...
    /* ── Signal ready → extension begins QEMU ────────────────────────── */
...
QemuLogPanel.currentPanel = undefined;
QemuLogPanel.viewType     = "qemuLogViewer";

exports.QemuLogPanel = QemuLogPanel;
