...
USER_BUILTIN_APPS := init hello fault ipc_recv ipc_send test_cow test_str cpu_stress simple test_exec test_lazy test_mutex test_pthread
...
ifeq ($(SMP_REGRESSION_TESTS),1)
CFLAGS += -DCONFIG_SMP_REGRESSION_TESTS=1
ASFLAGS += -DCONFIG_SMP_REGRESSION_TESTS=1
endif

ifeq ($(SCHED_REGRESSION_ONLY),1)
CFLAGS += -DCONFIG_SCHED_REGRESSION_ONLY=1
